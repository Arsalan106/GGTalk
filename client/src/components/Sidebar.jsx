    ...
    return (
      ...
      bg-[#282142] border border-gray-600 text-gray-100 hidden  group-hover:block'>
                ...
                <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
                    ...
                    <input type="text" placeholder="Search User" className='bg-transparent border-none outline-none text-white text-xs placeholder-[#c8c8c8] flex-1'></input>
                </div>
            </div>
            {/* display users */}
            <div className='flex flex-col mt-4'>
                {userDummyData.map((user, index) => (
                    <div onClick={() => { setSelectedUsers(user) }} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm
                    ${selectedUsers?._id === user._id && 'bg-[#282142]'}`}>
                        ...
                        {
                            index > 2 && <p className='absolute top-4 right-4 text-xs h-5 w-5 flex justify-center items-center rounded-full lg violet'>
                                {index}
                            </p>
                        }
                    </div>
